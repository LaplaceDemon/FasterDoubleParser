...
import io.github.laplacedemon.fasterdoubleparser.util.BytesUtil;
 ...
 * </dl>
 */
public class FasterDoubleParserFromByteArray {
...
//            MethodHandles.byteArrayViewVarHandle(long[].class, ByteOrder.LITTLE_ENDIAN);
     ...
     */
    private FasterDoubleParserFromByteArray() {
    ...
    private static double parseRestOfDecimalFloatLiteral(byte[] str, int endIndex, int index, boolean isNegative, boolean hasLeadingZero, int off) {
        ...
        for (; index < endIndex; index++) {
            ...
            if (isInteger(ch)) {
                ...
                while (index < endIndex - 9) {
                    long val = BytesUtil.readLongFromByteArrayGet(str, index + 1);
        ...
        Double result = FasterDoubleMath.decFloatLiteralToDouble(index, isNegative, digits, exponent, virtualIndexOfPoint, exp_number, isDigitsTruncated, skipCountInTruncatedDigits);
        if (result == null) {
    ...
    private static double parseRestOfHexFloatingPointLiteral(
        ...
        Double d = FasterDoubleMath.hexFloatLiteralToDouble(index, isNegative, digits, exponent, virtualIndexOfPoint, exp_number, isDigitsTruncated, skipCountInTruncatedDigits);
